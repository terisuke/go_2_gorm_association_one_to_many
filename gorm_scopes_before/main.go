...
func CashOrders(db *gorm.DB) *gorm.DB {
	return db.Where("payment_mode = ?", "cash")
}
...
func main() {
	...
	var orders []Order
	DB.Scopes(PriceGreaterThan30, CardOrders).Find(&orders)
	...
	var users []User
	DB.Preload("Orders", CardOrders).Scopes(UsersFromDomain("example.com")).Find(&users)
